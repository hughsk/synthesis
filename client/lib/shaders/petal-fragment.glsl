...
void main()
{
    ...
    if (lineDarkness != 0.0 && lines > 0.0) {
        // Bright points
        float line = 1. - abs(cos(lengthPoint * PI));
        // Darkening
        line *= lineDarkness;
        // Adding the lines
        line *= sin(widthPoint * lines);
        // Baseline
        line += 1.1;

        if (border > 0.0 && (widthPoint < borderSize * 2. - 1. || widthPoint > 1. - borderSize * 2.)) {
            c *= 1.0;
        } else
        if (line < 1.) {
            c *= line;
    ...
    if (border > 0.0) {
        if (widthPoint < borderSize - 1. || widthPoint > 1. - borderSize) {
            c *= (1. - border);
        }
    }

    if (dirtiness > 0.0) {
        c *= 1.0 + dirtiness * (snoise(
            vec2(widthPoint * 4., growth * lengthPoint * 18.)
        ) + snoise(
            vec2(widthPoint * 12., growth * lengthPoint * 40.)
        ));
    }
